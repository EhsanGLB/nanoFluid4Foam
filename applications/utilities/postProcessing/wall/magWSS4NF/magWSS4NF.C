...
)
{
    ...
    forAll(magWSS4NF.boundaryField(), patchI)
    {
        magWSS4NF.boundaryField()[patchI] = mag(
        (
...
int main(int argc, char *argv[])
{
    ...
    forAll(timeDirs, timeI)
    {
        ...
        if (UHeader.headerOk())
        {
            ...
            if (compressible)
            {
                calcCompressible(mesh, runTime, U, magWSS4NF);
            }
            else
            {
                calcIncompressible(mesh, runTime, U, magWSS4NF);
            }
        }
        else
        {
        ...
        Info<< "Writing wall shear stress to field " << magWSS4NF.name()
            << nl << endl;
