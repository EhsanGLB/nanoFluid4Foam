...
int main(int argc, char *argv[])
{
    ...
    forAll(timeDirs, timeI)
    {
        ...
        dimensionedScalar epsWSS = dimensionedScalar("epsWSS",TAWSS4NF.dimensions(), 1e-64);
        OSI4NF =  0.5 * ( 1 - mag(TAWSS4NF) / (TAWSSMag4NF+epsWSS) );
        RRT4NF = 1 /(( (1 - 2.0*OSI4NF) * TAWSSMag4NF )+epsWSS);
        RRT4NF.write();
        nTime++;
    }
