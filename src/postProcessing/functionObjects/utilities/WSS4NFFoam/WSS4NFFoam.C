...
namespace Foam
{
    defineTypeNameAndDebug(WSS4NFFoam, 0);
}


// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::WSS4NFFoam::writeFileHeader(const label i)
{
...
void Foam::WSS4NFFoam::calcShearStress
(
    const fvMesh& mesh,
    ...
    rhoName_("rho")
{
    ...
    if (active_)
    {
        ...
        mesh.objectRegistry::store(WSS4NFFoamPtr);
    }
...
void Foam::WSS4NFFoam::read(const dictionary& dict)
{
    if (active_)
    {
        ...
        else
        {
            ...
            forAllConstIter(labelHashSet, patchSet_, iter)
            {
                ...
                {
                    WarningIn("void WSS4NFFoam::read(const dictionary&)")
...
void Foam::WSS4NFFoam::execute()
{
    ...
    if (active_)
    {
        ...
        {
            FatalErrorIn("void Foam::WSS4NFFoam::execute()")
                << "Unable to find turbulence model in the "
                << "database" << exit(FatalError);
        }*/
            Reff = -mu*dev(twoSymm(fvc::grad(U)));

        calcShearStress(mesh, Reff(), WSS4NFFoam);
    }
}


void Foam::WSS4NFFoam::end()
{
    if (active_)
    {
        execute();
...
void Foam::WSS4NFFoam::timeSet()
{
    // Do nothing
}


void Foam::WSS4NFFoam::write()
{
    if (active_)
    {
        functionObjectFile::write();

        const volVectorField& WSS4NFFoam =
            obr_.lookupObject<volVectorField>(type());

        if (log_) Info<< type() << " " << name_ << " output:" << nl
            << "    writing field " << WSS4NFFoam.name() << nl
            << endl;
