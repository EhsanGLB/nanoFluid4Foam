...
{
    defineTypeNameAndDebug(HamiltonCrosser, 0);
    addToRunTimeSelectionTable(monoThermalConductivityModel, HamiltonCrosser, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

HamiltonCrosser::HamiltonCrosser
(
...
:
    ...
    HamiltonCrosserCoeffs_(nanoFluidPropertiesDict.subDict(typeName + "Coeffs")),
    lambda_(readScalar(HamiltonCrosserCoeffs_.lookup("lambda")))
{}


// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

const volScalarField HamiltonCrosser::kappa() const
{
    ...
    kappa_ = ( ( kappanp_ + ( lambda_ - 1 ) * kappabf_ - ( lambda_ - 1 ) * ( kappabf_ - kappanp_ ) * alpha_ ) / ( kappanp_ + ( lambda_ - 1 ) * kappabf_ + ( kappabf_ - kappanp_ ) * alpha_ ) ) * kappabf_;
