...
{
    defineTypeNameAndDebug(Evans, 0);
    addToRunTimeSelectionTable(monoThermalConductivityModel, Evans, dictionary);
...
:
    ...
    EvansCoeffs_(nanoFluidPropertiesDict.subDict(typeName + "Coeffs")),
    gamma_(readScalar(EvansCoeffs_.lookup("gamma")))
{}


// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

const volScalarField Evans::kappa() const
{
    ...
    kappa_ = ( 1 + 3 * alpha_ * ( ( gamma_ - 1) / ( gamma_ + 2 ) ) ) * kappabf_;
