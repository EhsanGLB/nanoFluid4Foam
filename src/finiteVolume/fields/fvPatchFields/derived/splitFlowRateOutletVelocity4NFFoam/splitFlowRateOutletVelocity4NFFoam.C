...
#include "splitFlowRateOutletVelocity4NFFoam.H"
#include "volFields.H"
#include "one.H"
...
Foam::splitFlowRateOutletVelocity4NFFoam::
splitFlowRateOutletVelocity4NFFoam
(
    const fvPatch& p,
...
splitFlowRateOutletVelocity4NFFoam
(
    const splitFlowRateOutletVelocity4NFFoam& ptf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
void Foam::splitFlowRateOutletVelocity4NFFoam::updateValues
(
...
void Foam::splitFlowRateOutletVelocity4NFFoam::updateCoeffs()
{
    if (updated())
    {
        return;
...
void Foam::splitFlowRateOutletVelocity4NFFoam::write(Ostream& os) const
{
    fvPatchVectorField::write(os);
    os.writeKeyword("inletPatch") << inletPatchName_ << token::END_STATEMENT << nl;
    os.writeKeyword("flowSplit") << flowSplit_ << token::END_STATEMENT << nl;

    if (!volumetric_)
    {
        os.writeKeyword("volumetric") << volumetric_ << token::END_STATEMENT << nl;
