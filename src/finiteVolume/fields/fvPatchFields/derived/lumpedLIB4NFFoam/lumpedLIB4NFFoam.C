...
Foam::lumpedLIB4NFFoam::lumpedLIB4NFFoam
(
    const fvPatch& p,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
Foam::scalar Foam::lumpedLIB4NFFoam::resistanceFunction(scalar LIBTemp)
{
    int n_ = 0;
    scalar f_ = 0.0;
...
Foam::scalar Foam::lumpedLIB4NFFoam::currentPolynomialFunction(scalar t)
{
    int n_ = 0;
    scalar f_ = 0.0;
...
Foam::complex Foam::lumpedLIB4NFFoam::currentWavyFunction(scalar t)
{
...
Foam::scalar Foam::lumpedLIB4NFFoam::currentFunction(scalar t)
{
    scalar f_ = 0.0;
    if (currentForm_ == "polynomial")
    {
...
void Foam::lumpedLIB4NFFoam::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    scalar symCoeff_ = 1.0;
    if (sym_ == "yes"){symCoeff_ = 2.0;}
    ...
    if(t_<10){QLoss_ = 0.0;}
    scalar Tb_ = initTemp_ + deltaT_*( (Qb_+symCoeff_*QLoss_) / (rho_*Cp_*volume_) );
    initTemp_ = Tb_;
...
// Write
void Foam::lumpedLIB4NFFoam::write(Ostream& os) const
{
    fvPatchScalarField::write(os);
    os.writeKeyword("state") << state_ << token::END_STATEMENT << nl;
    os.writeKeyword("sym") << sym_ << token::END_STATEMENT << nl;
...
namespace Foam
{
makePatchTypeField(fvPatchScalarField, lumpedLIB4NFFoam);
}
