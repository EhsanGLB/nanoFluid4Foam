...
{
    defineTypeNameAndDebug(Talbot, 0);
    addToRunTimeSelectionTable(thermophoresisDiffusivityModel, Talbot, dictionary);
...
:
    ...
    TalbotCoeffs_(nanoFluidPropertiesDict.subDict(typeName + "Coeffs")),
    Cs_(readScalar(TalbotCoeffs_.lookup("Cs"))),
    Cm_(readScalar(TalbotCoeffs_.lookup("Cm"))),
    Ct_(readScalar(TalbotCoeffs_.lookup("Ct"))),
    lbf_(readScalar(TalbotCoeffs_.lookup("lbf"))),
    dnp_(TalbotCoeffs_.lookup("dnp")),
    DTs_(particlesProperties.size())
{
    ...
    forAll(DTs_, i)
    {
        ...
        volScalarField beta_ = ( 2.0 * Cs_ * ( kappabf_ + particlesProperties_[i].kappa() * Kn_ ) * ( 1.0 + Kn_ * ( 1.2 + 0.41 * exp( -0.88 / Kn_ ) ) ) ) / ( ( 1.0 + 3.0 * Cm_ * Kn_ ) * ( 2.0 * kappabf_ + particlesProperties_[i].kappa() + 2.0 * particlesProperties_[i].kappa() * Ct_ * Kn_ ) );
