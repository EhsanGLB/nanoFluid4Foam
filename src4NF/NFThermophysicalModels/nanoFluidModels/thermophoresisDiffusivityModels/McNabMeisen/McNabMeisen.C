...
{
    defineTypeNameAndDebug(McNabMeisen, 0);
    addToRunTimeSelectionTable(thermophoresisDiffusivityModel, McNabMeisen, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

McNabMeisen::McNabMeisen
(
...
:
    ...
    McNabMeisenCoeffs_(nanoFluidPropertiesDict.subDict(typeName + "Coeffs")),
    DTs_(particlesProperties.size())
{
    ...
    forAll(DTs_, i)
    {
        volScalarField beta_ = 0.26 * ( kappabf_ / (2 * kappabf_ + particlesProperties_[i].kappa() ) );
