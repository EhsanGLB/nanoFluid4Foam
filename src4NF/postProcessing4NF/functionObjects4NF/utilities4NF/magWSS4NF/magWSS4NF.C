...
namespace Foam
{
    defineTypeNameAndDebug(magWSS4NF, 0);
}


// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::magWSS4NF::writeFileHeader(const label i)
{
...
void Foam::magWSS4NF::calcWSS4NF
(
    const fvMesh& mesh,
    const volSymmTensorField& Reff,
    volScalarField& WSS4NF
)
{
    forAllConstIter(labelHashSet, patchSet_, iter)
    {
        ...
        scalarField& ssp = WSS4NF.boundaryField()[patchI];
    ...
    rhoName_("rho")
{
    ...
    if (active_)
    {
        ...
        mesh.objectRegistry::store(magWSS4NFPtr);
    }
...
void Foam::magWSS4NF::read(const dictionary& dict)
{
    if (active_)
    {
        ...
        else
        {
            ...
            forAllConstIter(labelHashSet, patchSet_, iter)
            {
                ...
                {
                    WarningIn("void magWSS4NF::read(const dictionary&)")
...
void Foam::magWSS4NF::execute()
{
    ...
    if (active_)
    {
        ...
        {
            FatalErrorIn("void Foam::magWSS4NF::execute()")
                << "Unable to find turbulence model in the "
                << "database" << exit(FatalError);
        }*/
            Reff = -mu*dev(twoSymm(fvc::grad(U)));

        calcWSS4NF(mesh, Reff(), magWSS4NF);
    }
}


void Foam::magWSS4NF::end()
{
    if (active_)
    {
        execute();
...
void Foam::magWSS4NF::timeSet()
{
    // Do nothing
}


void Foam::magWSS4NF::write()
{
    if (active_)
    {
        functionObjectFile::write();

        const volScalarField& magWSS4NF =
            obr_.lookupObject<volScalarField>(type());

        if (log_) Info<< type() << " " << name_ << " output:" << nl
            << "    writing field " << magWSS4NF.name() << nl
            << endl;
